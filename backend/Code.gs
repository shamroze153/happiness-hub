...
const HEADERS = {
  ...
  Orders: [
    'order_id', 'buyer_name', 'buyer_whatsapp', 'product_id', 'product_title',
    'amazon_order_id', 'screenshot_url', 'payment_method', 'payment_id',
    'delivery_screenshot_url', 'notes', 'agent_id', 'seller_id', 'status',
...
/**
 * action=trackOrder  params: whatsapp? OR order_id?
 * Public lookup — returns only sanitized fields (no payment_id / account details).
 */
function trackOrder(p) {
  if ((!p.whatsapp || !String(p.whatsapp).trim()) && (!p.order_id || !String(p.order_id).trim())) {
    return { success: false, error: 'Enter a WhatsApp number or an Order ID to track your order.' };
  }
  ...
  if (p.order_id && String(p.order_id).trim()) {
    ...
    const target = String(p.whatsapp).trim();
    matches = rows.filter(function (r) { return String(r.buyer_whatsapp).trim() === target; });
  }

  if (!matches.length) {
    return { success: false, error: 'No order found. Double-check your WhatsApp number or Order ID and try again.' };
  }
...
/**
 * action=getBuyerOrders  params: whatsapp
 * Full order history for "My Orders", plus stats including needs_delivery_proof.
 */
function getBuyerOrders(p) {
  const err = requireFields(p, ['whatsapp']);
  if (err) return { success: false, error: err };

  const target = String(p.whatsapp).trim();
  const rows = sheetToObjects(getSheet('Orders'));
  const orders = rows
    .filter(function (r) { return String(r.buyer_whatsapp).trim() === target; })
    .sort(function (a, b) {
      const da = safeDate(a.submitted_at), db = safeDate(b.submitted_at);
      if (da && db) return db - da;
      return 0;
...
/**
 * action=submitOrder
 * params: buyer_name, buyer_whatsapp, product_id, amazon_order_id, screenshot_url,
 *         payment_method, payment_id, notes?, agent_id?
 */
function submitOrder(p) {
  const err = requireFields(p, [
    'buyer_name', 'buyer_whatsapp', 'product_id', 'amazon_order_id',
    'screenshot_url', 'payment_method', 'payment_id'
  ]);
  if (err) return { success: false, error: err };
  ...
  const order = {
    ...
    buyer_name: String(p.buyer_name).trim(),
    buyer_whatsapp: String(p.buyer_whatsapp).trim(),
    product_id: String(p.product_id).trim(),
    product_title: product.title,
    amazon_order_id: String(p.amazon_order_id).trim(),
  ...
  logActivity(p.agent_id || order.buyer_whatsapp, p.agent_id ? 'agent' : 'buyer', 'submitOrder', {
    order_id: order.order_id, product_id: order.product_id, buyer_whatsapp: order.buyer_whatsapp
  });
...
/**
 * action=submitDeliveryProof  params: order_id, delivery_screenshot_url
