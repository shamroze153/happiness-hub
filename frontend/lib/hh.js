...
(function (global) {
  ...
  const API = 'https://script.google.com/macros/s/AKfycbz4myw35niVsrH73aB_2aa3biU9_fwUiGbM3IfA_pf2TsQAqEtjLrJPVnq_co-5a-Ua/exec';
  ...
  function renderOrderCard(order, options) {
    ...
    if (tracker.errorStep !== -1) {
      const note = order.seller_notes && String(order.seller_notes).trim()
        ...
        : 'Please check My Orders for details, or reach out via WhatsApp support.';
    ...
    html += '<div class="flex justify-between items-center flex-wrap gap-8 mb-8">' +
      ...
      '<div class="text-muted" style="font-size:12px;">#' + esc(order.order_id) + ' · ' + esc(timeAgo(order.submitted_at)) + '</div>' +
      '</div>' +
    ...
    html += '<div class="flex justify-between items-center flex-wrap gap-8 mt-16">' +
      '<div class="refund-method">💰 ' + fmt$(order.cashback_amount) + ' via ' + esc(order.payment_method || '—') + '</div>';
